...
class User {
    ...
    public function init() {
        ...
        $this->info_list      = $this->profile_info( $this->current_user_id );
    ...
    public function client_option_manager() {
        ...
        $atts = [
            'profile' => $this->profile_info( $client_id ),
    ...
    public function assign_expert() {
        $atts = [
            'client_id'     => boston_var( 'client_id' ),
            'tax_expert_id' => boston_var( 'tax_expert_id' ),
        ...
        $client_profile = $this->profile_info( $atts['client_id'] );
        $expert_profile = $this->profile_info( $atts['tax_expert_id'] );
        ...
        $data = [
            ...
            'tax_expert_id'   => $atts['tax_expert_id'],
            'tax_expert_name' => $expert_profile['full_name'],
            'assigned_at'     => time(),
        ];

        $updated = $this->db->update(
            "{$this->prefix}boston_currently_assigned_clients",
            $data,
            ['client_id' => $atts['client_id']],
            [
                '%d',
    ...
    public function assign_to_current_session( $user_name, $user ) {
        ...
        $user_info = $this->profile_info( $user_id );
    ...
    public function does_have_current_assigned_session( $user_id ) {
        $have = (int) $this->db->get_var(
            "SELECT count(id) FROM {$this->prefix}boston_currently_assigned_clients
    ...
    public function profile_info( $user_id = 0 ) {
        $info = [];
        ...
        $info['role_select']  = $this->account_type( $user->ID );
    ...
    public function boston_user_dashboard_contents() {
        switch ( $this->account_type ) {
            ...
            case 'tax expert':
                ...
                break;
            default:
                print_r( $this->profile_info( get_current_user_id() ) );
    ...
    public function tax_expert_dashboard_contents() {
        ...
        switch ( $tab ) {
            case 'clients':
                $clients = $this->clients_of_expert( $this->current_user_id );
    ...
    public function clients_of_expert( $user_id ) {
        $clients = $this->db->get_results(
            $this->db->prepare(
                "SELECT * FROM {$this->prefix}boston_currently_assigned_clients WHERE tax_expert_id={$user_id}"
            )
        );
        ...
        for ( $i = 0; $i < sizeof( $clients ); $i++ ) {
            $clients[$i]['profile'] = $this->profile_info( $clients[$i]['client_id'] );
        }
    ...
    public function status( $user_id ) {
        ...
        $status      = 'blue';
        $have_expert = $this->have_expert( $user_id );
    ...
    public function does_filed( $user_id ) {
        $result = $this->db->get_row(
            $this->db->prepare(
                "SELECT filed FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id}"
            )
        );
    ...
    public function assgined_clients_data_list( $args ) {
        ...
        $clients = $this->db->get_results(
            $this->db->prepare(
                "SELECT * FROM {$this->prefix}boston_currently_assigned_clients
    ...
    public function assigned_clients() {
        ...
        switch ( $action ) {
            case 'manage':
                ...
                $profile_info = $this->profile_info( $user_id );
    ...
    public function have_expert( $user_id ) {
        ...
        $have = $this->db->get_row(
            $this->db->prepare(
                "SELECT tax_expert_id FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id}"
            )
        )->tax_expert_id;
    ...
    public function get_priority( $user_id ) {
        return $this->db->get_row(
            $this->db->prepare(
                "SELECT priority FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id}"
            )
        );
    }
    ...
    public function delete_from_previous_year( $user_name, $user ) {
        $user_id = $user->ID;
        $this->db->query(
            "DELETE FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id} AND CAST(start_date as INT) < {$this->previous_year}"
        );
    }
    ...
    public function assigned_information( $user_id ) {
        return $this->db->get_row(
            $this->db->prepare(
                "SELECT * FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id} AND cast(start_date as INT) > {$this->previous_year}"
            )
        );
    }
    ...
    public function current_expert( $user_id ) {
        ...
        $expert = $this->db->get_row(
            $this->db->prepare(
                "SELECT * FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$user_id}"
            )
        );
    ...
    public function have_access_of_client( $client_id, $expert_id ) {
        ...
        $result = $this->db->get_row(
            $this->db->prepare(
                "SELECT * FROM {$this->prefix}boston_currently_assigned_clients WHERE client_id={$client_id}"
            )
        );

        if ( $result->client_id == $client_id && $result->tax_expert_id == $expert_id ) {
            return true;
        } else {
            return false;
        }
    ...
    public function client_short_profile( $client_id ) {
        ...
        $profile['current_tax_expert'] = $this->profile_info( $this->current_expert( $client_id ) );
    ...
    public function expert_short_profile( $expert_id ) {
        $profile                      = $this->profile_info( $expert_id );
