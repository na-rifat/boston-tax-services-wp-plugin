<?php
    ...
    class Sociallogin {
        ...
        public function __construct() {
            // add_action('boston_social_login_button', array( $this, 'show_buttons' ) );
            add_action( 'um_after_form', array( $this, 'show_buttons' ) );
            add_action( 'wp_enqueue_scripts', array( $this, 'load_scripts' ), 12 );

            $this->networks = array(
                'facebook' => array(
                    'title'  => esc_html__( 'Facebook', 'boston' ),
                    'data'   => array(),
                    'width'  => 48,
                    'height' => 48,
                ),
                'twitter'  => array(
                    'title'  => esc_html__( 'Twitter', 'boston' ),
                    'data'   => array(),
                    'width'  => 58,
                    'height' => 48,
                ),
                'google'   => array(
                    'title'  => esc_html__( 'Google', 'boston' ),
                    'data'   => array(),
                    'width'  => 70,
                    'height' => 70,
                ),
        ...
        public function start_social_login() {
            foreach ( $this->networks as $network => &$data ) {
                $id           = get_option( $network . '_app_id' );
                $secret       = get_option( $network . '_app_secret' );
                $data['data'] = array(
                    'id'     => $id,
                    'secret' => $secret,
                );
            ...
            if ( ! empty( $_GET['boston-connect'] ) ) {
                switch ( $_GET['boston-connect'] ) {
                    case 'facebook':$this->facebook_connect();
                        break;
                    case 'twitter':$this->twitter_connect();
                        break;
                    case 'google':$this->google_connect();
                        break;
                }
            }
        ...
        public function callback() {
            if ( ! empty( $_GET['boston-callback'] ) ) {
                switch ( $_GET['boston-callback'] ) {
                    case 'facebook':$this->facebook_callback();
                        break;
                    case 'twitter':$this->twitter_callback();
                        break;
                    case 'google':$this->google_callback();
                        break;
                }
            }
        }
        ...
        public function load_scripts() {
            // wp_localize_script( 'boston-custom', 'boston_sc', array(
        ...
        public function show_buttons() {
                    ...
                    foreach ( $this->networks as $network => $data ) {
						...
						<li>
							<a href="javascript:void(0);" class="<?php echo esc_attr( $network ) ?>">
								<img src="<?php echo imgfile( "{$network}.png" ) ?>" width="<?php echo esc_attr( $data['width'] ) ?>" height="<?php echo esc_attr( $data['height'] ) ?>">
			...
			<?php
                    ...
                    private function _login_user( $userdata ) {
                        ...
                        if ( ! empty( $user_id ) ) {
                            update_user_meta( $user_id, 'boston_avatar_url', $userdata['user_avatar'] );
                        }
		...
		<?php
                ...
                public function facebook_connect() {
                    wp_redirect( 'https://graph.facebook.com/oauth/authorize?scope=email&client_id=' . esc_attr( $this->networks['facebook']['data']['id'] ) . '&redirect_uri=' . site_url( 'index.php?boston-callback=facebook' ) );
                    die();
                }
                ...
                public function facebook_callback() {
                    if ( isset( $_GET['code'] ) ) {
                        $response = $this->_http_facebook( 'oauth/access_token', array(
                            ...
                            'redirect_uri'  => site_url( 'index.php?boston-callback=facebook' ),
                        ...
                        if ( ! empty( $response->access_token ) ) {
                            ...
                            if ( ! empty( $user->id ) ) {
                                ...
                                esc_html_e( 'Could not retrieve user data', 'boston' );
                            }
                        } else {
                            esc_html_e( 'Could not generate access_token', 'boston' );
                        }
                    } else {
                ...
                public function twitter_connect() {
                    $response = $this->_http_twitter( 'oauth/request_token', array(
                        'oauth_callback' => site_url( '/index.php?boston-callback=twitter' ),
                    ) );
                    ...
                    if ( ! empty( $response['oauth_callback_confirmed'] ) && $response['oauth_callback_confirmed'] == true ) {
                        ...
                        esc_html_e( 'Could not generate oauth_token since credentials are invalid', 'boston' );
                    }
                    die();
                }
                ...
                public function twitter_callback() {
                    if ( isset( $_GET['oauth_token'] ) && isset( $_GET['oauth_verifier'] ) ) {
                        ...
                        if ( ! empty( $response['oauth_token'] ) && ! empty( $response['oauth_token_secret'] ) ) {
                            ...
                            esc_html_e( 'Could not generate oauth_token_secret since credentials are invalid', 'boston' );
                        }

                    } else {
                        esc_html_e( 'Login error', 'boston' );
                    }
                    die();
                }
                ...
                public function google_connect() {
                    $data = array(
                        ...
                        'redirect_uri'  => site_url( 'index.php?boston-callback=google' ),
                ...
                public function google_callback() {
                    if ( isset( $_GET['code'] ) ) {
                        ...
                        if ( ! empty( $response->access_token ) ) {
                            ...
                            esc_html_e( 'Could not retrieve access_token', 'boston' );
                        }
                    } else {
                        esc_html_e( 'Login error', 'boston' );
                    }
